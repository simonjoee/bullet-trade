# ZZ：ETF 动量轮动交易，代码略复杂
#
# 通过轮动投资于不同的核心资产ETF，并结合止损机制来控制风险，追求高收益低回撤

# 1. 资产池设置
# 策略选择了4个不同类型的ETF作为投资标的：
# - 518880.XSHG - 黄金ETF（大宗商品）
# - 513100.XSHG - 纳指100（海外资产）
# - 159915.XSHE - 创业板100（成长股、科技股、中小盘）
# - 510180.XSHG - 上证180（价值股、蓝筹股、中大盘）
# 这种配置覆盖了不同的资产类别和市场风格，有助于分散风险。
#
# 2. 动量因子计算（get_rank）
# 使用动量因子来选择要投资的ETF，动量因子由年化收益率和判定系数(R²)相乘得到。这样既考虑了趋势的强度，也考虑了趋势的稳定性。
#
# 3. 交易逻辑
# 策略每天运行，但实际交易发生需要满足特定条件：
# - 卖出条件：
#   - 当前持有的ETF不在新的最高动量ETF列表中（轮动调仓）
#   - 当前持有的ETF触发止损条件（当日跌幅超过4%）
# - 买入条件：
#   - 选择的新ETF与之前持有的ETF不同（轮动）
#   - 或者在没有持仓的情况下，新选择的ETF符合买入条件
