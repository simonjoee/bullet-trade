# ZZ：
# 动量因子（Momentum Factor）：是指股票或其他资产在一段时间内表现出的趋势延续性。
# - 上涨趋势：过去表现好的资产，未来一段时间可能继续表现好
# - 下跌趋势：过去表现差的资产，未来一段时间可能继续表现差
# 小市值反转因子（Small Cap Reversal Factor）：过去表现差的小市值股票，未来可能反弹（反转）
# - 过度反应后会出现修正
# - 便宜的小市值股票可能被低估，有反转机会

# 策略特点
# - 动量轮动：每月选择过去34天动量最强的ETF， 动量因子计算 R²，得分 = 年化收益率 × R²
# - EPO优化：使用EPO（Enhanced Portfolio Optimization）方法优化权重分配
# - 多样化：ETF池覆盖商品、海外、宽基、窄基等多个类别
