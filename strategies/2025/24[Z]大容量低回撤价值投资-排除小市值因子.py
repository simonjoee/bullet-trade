# ZZ： 价值投资策略
# 基于价值投资理念的量化投资策略，主要目标是通过多因子模型选股来实现大容量低回撤的投资收益。未带个股止损？？
#
# 1. 多因子选股模型
# 策略使用四个关键因子进行股票筛选：
# - roic_ttm：投资资本回报率TTM（正向因子）
# - gross_income_ratio：销售毛利率（正向因子）
# - sales_to_price_ratio：营收市值比（负向因子，等同于市销率的倒数）
# - Variance120：120日年化收益方差（负向因子，衡量风险）
# 体现了价值投资理念：优选盈利能力强、毛利率高、估值合理（市销率低）、风险小（波动小）的股票。
#
# 2. 交易执行机制
# - 季度调仓：仅在1、4、7、10月进行调仓
# - 持仓管理：卖出不在目标股票池且未涨停的股票
# - 资金分配：剩余资金平均分配给新买入的股票
# - 涨停板处理：对持仓中的涨停股进行特殊管理，若涨停打开则卖出
# - 历史持仓限制：最近持有过的股票20天内不再买入
