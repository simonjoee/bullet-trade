...
# 标题：首板低开策略
# 1. 选股条件：选择前一日涨停但非连板的股票，且股价处于最近60日相对低位（在中位数以下）。
# 2. 买入时机：次日低开3%-4%时买入（开盘价为昨日收盘价的96%-97%），等权重买入所有符合条件的股票。
# 3. 止盈操作：上午11:28若股价高于买入均价且未封涨停则卖出。
# 4. 止损操作：下午14:50无论盈亏，对未封涨停的股票全部卖出。
# 5. 股票池过滤：排除科创板、北交所、ST股、上市不足250天及停牌股票。
# 作者：wywy1995
